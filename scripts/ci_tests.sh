...
do
  ...
  set +e
  docker run --rm -e INPUT_DIR=/app/${test_name} -e OUTPUT_DIR=/output/${test_name} -e ROM_DIR=/output -e ROM_FILE=main.gb -e DELAY=40 -e BGB_TIMEOUT=45 -v `pwd`:/output stickrpg-automated-test-tool:latest
  if [ "$?" != "0" ]
  then
    tests_failed=1
  fi
